...
- ### **Modes (`powersave` / `balance` / `performance` / `fast`) Description:**
  ...
  - #### **Mode Switching:**
    ...
    - If you have some understanding of programming on Linux, you can switch to the corresponding mode by writing any of the 4 modes to the `/data/adb/fas-rs/dev/mode` node, and you can also read it to know the current mode of `fas-rs`.
